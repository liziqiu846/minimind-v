...
|---|---|---|
| `ACTIVE` | `PROJALLOC-01` | LITMAP-03 找到 connector、frozen encoder 与 vision unfreezing 的直接但方向冲突证据；用固定总 4,096 coordinates 的 projector allocation 最小干预裁决 |
| `NEXT` | `PROJALLOC-01_PAIRED_PILOT` | 先以 fresh root `43201` 重训 current 与 projector-dominant；只有 mechanism 与 external 两门同时阳性才补 `43202/43203` |
