...
|---|---|---|
| `ACTIVE` | `LITMAP-02` | `COMP-01`、`XMC-01`、`VISCOND-01` 三条冻结 checkpoint 路线均被否定；先以失败证据约束定向文献/理论 gate，再登记真正不同的新 candidate |
| `NEXT` | `TBD_FROM_LITMAP-02` | 只接受不复用已失败生成 NLL/no-pixel/无桥 representation proxy，且能产生最小判别实验的 VLM-specific mechanism |
