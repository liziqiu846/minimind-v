...
|---|---|---|
| `ACTIVE` | `LITMAP-04` | `VISSUP-01` 与 fixed-total `PROJALLOC-01` 均未使 held-out mechanism 或外部任务改善；必须用失败证据检索 objective competition / gradient routing、task-specific absorption 与 frozen-feature/AR-objective mismatch 的真正不同机制 |
| `NEXT` | — | 当前没有已通过初筛且可直接执行的新 candidate；`NEXT<2` 已触发定向文献检索 |
| `BACKLOG` | `OBJ-01`, `COVER-01` | 原 OBJ 启动依赖的 VISCOND positive gate 已失败；COVER 缺权威 domain/mixture 标签，只有 LITMAP-04 提供新理论缺口、反例与可靠文献后才能重审，不得无机制地直接训练 |
