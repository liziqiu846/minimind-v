...
|---|---|---|
| `ACTIVE` | `CROSSFACT-01` authoritative crossed-cell gate | `COVER-01` 已证明 broad source/task labels 与 transfer、difficulty、output format、quality 和 target choice 缠结；改查同一 acquisition/image unit 上由发布者预先定义的 crossed text/task factors，避免换名重复 mixture search |
