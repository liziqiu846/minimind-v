...
| `LITMAP-07` | `LITERATURE_SCREEN` | XID neural failure 后 factorized absorption vs trainability 重选 | completed; selected `VSTATE-01` | 2,455 raw / 1,242 unique、11 篇全文；选择 shared visual-state gradient mediation，只证明值得建立 theory bridge |
| `XID-01` | `SCIENTIFIC_MECHANISM` | autoregressive supervision 下的跨模态交互规则可识别性 | `CONCLUSION_CANDIDATE` theory; empirical instantiation rejected | round1–3 finite theory proven；round4 valid MiniMind-V pilot 强拒绝当前 neural bridge，禁止同 task rescue |
| `VSTATE-01` | `SCIENTIFIC_MECHANISM` | answer-only AR supervision 下共享视觉状态的梯度中介与规则分解 | `ACTIVE_CONJECTURE` | direct VLM evidence 支持显式 image-to-text/state supervision 改变 unseen transfer；下一步先证明 key-wise gradient cancellation / shared-state non-cancellation，representation ceiling 保留为竞争解释 |

## Dynamic backlog

| Queue | Candidate | Why now? |
|---|---|---|
| `ACTIVE` | `VSTATE-01` shared visual-state mediation of autoregressive gradients | XID valid failure + Park et al. direct VLM intervention expose a precise identifiability–optimization bridge: answer-only key-wise gradients may not form a reusable visual state |
| `NEXT` | representation / 4,096-coordinate ceiling；generic AR visual-credit competition | 前者是 VSTATE 的明确 competing explanation；后者有 independent evidence，但只有在 VSTATE 理论/预测失败后才重新拆分 |
| `BACKLOG` | cross-modal compositional factorization；joint multimodal support coverage；`OBJ-01` | 保留上位机制，不用新 proxy、module allocation 或 broad data labels提前 operationalize |
...
| XID-01 | autoregressive supervision 下的 cross-modal interaction identifiability | Data + learning theory + training | 图像与语言上下文可对同一 next-token target 提供 observationally equivalent 的解释；只含 token sequence 的普通 LLM 不具有跨模态 interaction-rule ambiguity | `Words or Vision`、ComPABench、controlled CLIP coverage 和 multimodal contrastive identifiability 分别覆盖相邻现象/理论，但没有 formal result 处理 discrete AR conditional predictors 在 observed multimodal support 上等价、在 unseen cells 分歧 | checkpoint 码长与 observed NLL 都不能表示训练支持是否排除了 language shortcut 或错误 composition rule；相同复杂度模型可因识别到不同 rule 而有不同 target risk | theorem prediction：\(\lambda>(\beta+2\alpha_n)/(\beta+\gamma)\) 排除 target-bad shortcut；redundant \(\gamma=0\) 不能只靠重复获得 strict guarantee | round1–3 theory complete；round4 root 43301：primary consistent−ambiguous=`-48.21 pp`、CI `[-51.69,-44.84] pp`；mechanism diff=`+0.66 pp`、两条件 full-rule success=0；全部 engineering gates pass | interaction-identifying sampling、counterfactual support cells、能够区分 shortcut 与 intended interaction 的辅助 objective | Theory 保留为 `CONCLUSION_CANDIDATE`；当前 synthetic M2 empirical instantiation 被拒绝，禁止 roots 43302/43303 与调参 rescue；next search 指向 factorized-rule absorption vs trainability ceiling | THEORY_CONCLUSION_CANDIDATE / INSTANTIATION_REJECTED |
| VSTATE-01 | answer-only autoregressive supervision 下，共享视觉状态是否通过 non-cancelling gradient mediation 形成可迁移跨 key rule | Training + representation + learning theory | 同一视觉因素必须跨语言 key 复用；terminal answer residual 可随 key 改变方向，而 key-invariant state target 可提供共享视觉更新 | Park et al. 2501.02669 的 explicit image-to-text intervention + gradient theorems；Words or Vision 的 matched mixture；OGM/PMR 的 modality under-optimization；Cambrian/Idefics2 保留 trainability ceiling；现有 work 未证明 shared-rule recovery | population support 可识别规则仍不保证 optimizer 形成共享 visual state；这正是 XID theory-to-neural failure 所暴露的缺口 | 在固定 representation/coordinates/pixels/support/final answer 时，key-invariant visual-state supervision 应同时提高 state prediction、完整 cross-key rule 与 unseen composition；若 representation ceiling 成立则三者共同失败 | round1 先做最小 factorized AR gradient identity 与 exhaustive verifier；未通过 theory 不训练 | explicit/internalized visual-state targets、factorized intermediate supervision、alignment-first curriculum | Why now：XID round4 valid rejection + 两个独立 direct VLM sources；不是改 marker/ratio/LR 的 rescue，也不把 gradient alignment score当 proxy | ACTIVE_CONJECTURE |
| LITMAP-07 | XID failure-targeted neural factorization search | Literature/theory gate | 区分 shared rule absorption、AR visual credit 与 representation/trainability ceiling | 16 query families、2,455 raw、1,242 unique、11 篇决定性全文 | 把“factorization”从现象收缩成 visual-state-mediated gradient mechanism，并保留 ceiling 作为 competing explanation | 文献必须导出无需 layer/rank/ratio/proxy search 的 directional prediction | 0 GPU/checkpoint/training；full-text/theorem applicability audit | 通过时先 theory bridge，再决定 matched intervention | selected `VSTATE-01`; no figure per user instruction; all source hashes and deterministic index verified | VALID |
...
- **What exactly is rejected**: 当前 synthetic border/key、M2-current、
  4,096-coordinate、1,040/11,040 injection、root 43301 的 matched-support
  empirical mechanism instantiation。
- **What is NOT rejected**: round1–3 finite interaction-identifiability theorem、
  diagnostic support 作为 population identification mechanism、其他有独立依据的
  factorized architecture/objective，或自然任务中的 cross-modal composition。
- **Evidence**: primary accuracy ambiguous/consistent=`92.46%/44.25%`，
  difference=`-48.21 pp`、95% CI `[-51.69,-44.84] pp`；mechanism
  accuracy=`49.81%/50.47%`，difference=`+0.66 pp`，两条件 full-rule success=0；
  全部 paired engineering invariants 通过。
- **Applicable scope**: one paired root under the exact frozen data, marker,
  prompt, optimizer, coordinates and scorer. 单 seed 按预注册规则足以拒绝当前
  instantiation，不支持普遍 seed-effect 估计。
- **What this rules out**: “只要 population support 唯一偏好 intended rule，当前
  low-dimensional MiniMind-V optimizer 就会形成 shared cross-key rule”这一具体
  bridge。
- **What remains possible**: key-specific task absorption、representation ceiling、
  optimization bias、需要明确 factorization bias 的 objective/architecture。
- **Next search implication**: 不补 roots、不改 synthetic task；检索能区分
  factorized rule formation 与 feature/trainability ceiling 的 formal/direct LVLM
  evidence，再选择新机制。
