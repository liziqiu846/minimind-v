...
| COVER-01 | 图文域与组合覆盖，而非样本量本身 | Data | 联合覆盖对象是“域×视觉概念×文本表达”的跨模态组合 | Kempf et al. 2025、DataComp、MM1；本轮按 immutable gate 重新核查 direct generative-LVLM bridge 与 official dataset lineage | 短码模型仍可能在未覆盖组合上失败；简单规模/码长无法编码覆盖关系 | 固定总训练预算时，source-defined complementary coverage 应比同域 redundancy 改善预声明 held-out stratum，且不能由 quality/difficulty/caption leakage 解释 | round1 只核查 authoritative strata、exact baseline lineage、single-factor contrast、held-out prediction 与本地可行性；支持后另建训练 plan | coverage-aware sampling、dataset mixture optimization、targeted recaptioning | Why now：连续两个 instantiation 与两个 bridge 暴露 analyst-choice/identifiability 问题；source-defined data strata 可避免新 checkpoint proxy | TESTING |
