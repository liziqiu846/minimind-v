...
|---|---|---|
| `ACTIVE` | `COMP-01` | `XMC-01_round1` 已排除已审计 P/S 的纯数据差异；标准外部关系反转 panel 可最低成本检验模型是否保留组合绑定 |
| `NEXT` | `XMC-01` model-retention bridge | 纯数据版本局部排除，但正式多模态谱理论使“模型保留关系结构”仍有理论价值 |
| `NEXT` | `VISCOND-01` | 若组合绑定不能解释性能排序，视觉条件利用是最直接的生成式 LVLM 特有 competing mechanism |
| `BACKLOG` | `OBJ-01` | 需要最小训练区分 next-token 监督与关系辅助目标，先等待 checkpoint-only evidence |
