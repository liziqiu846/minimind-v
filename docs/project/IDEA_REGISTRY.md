...
|---|---|---|
| `ACTIVE` | `LITMAP-03` | VISSUP 的视觉 condition 连 held-out rotation 都未优于泄露 control；先检索低维 PEFT 中 frozen-encoder identifiability、trainable-subspace capacity 与 objective competition 的可区分证据 |
| `NEXT` | `TBD_FROM_LITMAP-03` | 只接受不换 rotation task/ratio/proxy rescue，且能用唯一最小干预区分至少两个失败解释的新 VLM-specific candidate |
