...
| 2026-08-07 | XID-01 matched-support MiniMind-V mechanism pilot | matched pixels/marginals/entropy/compute 下，consistent interaction support 是否优于 ambiguous visual-necessary support 泛化到 held-out marker×key | plan `5cbe500`; implementation/preflight this commit | common 10k captions + 1,040 ten-row blocks；M2-current；ambiguous vs consistent；root 43301；1,008-group target + factorial panel | data/token/pixel/model/resource preflight 全通过；parquet SHA 固化；4,096 zero coordinates、11 targets、22 factor mappings、2,070 steps 均核验；0 inference/training | `AUDIT_PASS` | `experiments/plans/XID-01_round4.md`; `experiments/results/XID-01_round4/PREFLIGHT.json`；下一步只允许 paired smoke，positive pilot 才补 roots |
