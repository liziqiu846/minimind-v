...
| 2026-08-07 | XID-01 matched-support MiniMind-V mechanism pilot | matched pixels/marginals/entropy/compute 下，consistent interaction support 是否优于 ambiguous visual-necessary support 泛化到 held-out marker×key | current plan commit | common 10k captions + 1,040 ten-row blocks；M2-current；ambiguous vs consistent；root 43301；1,008-group target + factorial panel | immutable plan 由本次 commit 冻结；implementation/preflight 后才可 smoke/train | `PROPOSED` | `experiments/plans/XID-01_round4.md`；pilot 2 trainings，positive-only total max 6；no final confirmation |
