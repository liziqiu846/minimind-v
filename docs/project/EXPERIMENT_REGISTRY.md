...
| 2026-08-07 | XID-01 diagnostic-mass prediction theorem | diagnostic mass/separation 能否以预声明阈值排除 target-bad shortcuts，并区分 redundancy 与 identification | current plan commit | \(\lambda,\gamma,\beta,\alpha_n\) sufficient threshold；sharpness construction；fixed rational grid；round1 mapping | immutable prediction plan 由本次 commit 冻结；执行前不写 theorem/proof 或运行 verifier | `PROPOSED` | `experiments/plans/XID-01_round3.md`；0 checkpoint/GPU/training/final confirmation |
