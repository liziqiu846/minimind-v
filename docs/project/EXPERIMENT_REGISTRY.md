...
| 2026-08-07 | XID-01 matched-support MiniMind-V mechanism pilot | matched pixels/marginals/entropy/compute 下，consistent interaction support 是否优于 ambiguous visual-necessary support 泛化到 held-out marker×key | plan `5cbe500`; implementation/preflight `cf815e4`; smoke this commit | common 10k captions + 1,040 ten-row blocks；M2-current；ambiguous vs consistent；root 43301；1,008-group target + factorial panel | preflight 全通过；二条件各 2-sample smoke 的 loss/gradient exact match 且 finite，未保存 coordinates、未计算科学结果 | `AUDIT_PASS` | `experiments/results/XID-01_round4/PREFLIGHT.json`; `SMOKE.json`；下一步按 frozen order 运行 root 43301，positive pilot 才补 roots |
