...
| 2026-08-07 | LITMAP-05 frozen-feature sufficiency / identifiability gate | 是否有 architecture/theory 唯一固定的 frozen-feature readout，可区分 representation-absent 与 downstream-unabsorbed | current plan commit | formal readout/identifiability theory、direct frozen-encoder VLM/LVLM controls、exact local interface | immutable plan 由本次 commit 冻结；只执行 literature/theory 与 read-only interface gate | `PROPOSED` | `experiments/plans/LITMAP-05_round1.md`；不运行 checkpoint readout 或 training |
