...
| 2026-08-07 | XID-01 finite-hypothesis target-risk decomposition | source ERM target excess risk 能否分为 approximation、alignment、exact identification diameter 与 estimation expansion | current plan commit | finite bounded-loss class；uniform convergence；fixed 5-point risk grid × 3 deviation radii；round1 specialization | immutable plan 由本次 commit 冻结；执行前不写 theorem/proof 或运行 verifier | `PROPOSED` | `experiments/plans/XID-01_round2.md`；0 checkpoint/GPU/training/final confirmation |
