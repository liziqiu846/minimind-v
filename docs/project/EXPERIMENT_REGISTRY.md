...
| 2026-08-07 | COVER-01 authoritative controlled-coverage gate | 是否存在 source-defined authoritative strata 与 exact local lineage，可形成 complementary-coverage vs same-domain-redundancy 的唯一单因素 contrast | plan `cc88cd3`; result this commit | 442 raw records、380 unique titles、69 prior-search duplicates、75 score≥10；14 篇决定性 primary sources；official MiniMind/ALLaVA schema/license/tree；1,274,698-row local parquet 与 169 official sample exact-match audit | direct generative evidence 依赖 ratio/category/target search 或复合 curation；cleanest controlled evidence 仅为 CLIP；local sample lineage 可部分恢复，但 broad source/task labels 与 acquisition/task/style/quality/difficulty/output format 同变；`NO_CANDIDATE` | `BRIDGE_REJECTED` | 只否定当前 authoritative-broad-label-to-local-single-factor coverage bridge，不否定 coverage mechanism。0 checkpoint/GPU/training。`experiments/results/COVER-01_round1/` |
| 2026-08-07 | CROSSFACT-01 authoritative crossed-cell gate | 是否存在同一 authoritative image/acquisition unit × publisher-defined text/task factors 的 systematic crossing，可唯一冻结 four-way cells 与 generative held-out prediction | current plan commit | targeted direct generative crossed-cell evidence、formal adjacent theory、2–4 official dataset schemas、local same-asset lineage | immutable plan 由本次 commit 冻结；round1 只执行 literature/schema/local-lineage gate，0 checkpoint/GPU/training | `PROPOSED` | `experiments/plans/CROSSFACT-01_round1.md`；通过全部 source-factor orthogonality 与 exact-cell gates 后才允许登记一个 training candidate |
| 2026-08-07 | LITMAP-06 failure-informed scientific-mechanism reselection | 三个上位搜索起点及其合流机制中，哪个具有最高 scientific expected value | plan `1c63dbf`; result this commit | 3,479 records、2,395 unique titles、10 篇决定性全文；AR credit、composition、joint support、interaction identifiability | 选择 `XID-01` cross-modal interaction identifiability；现有 formal work 未覆盖 discrete autoregressive conditional-rule equivalence；仅为 `CONJECTURE` + `EMPIRICAL_SUPPORT` | `VALID` | `experiments/results/LITMAP-06_round1/`；0 checkpoint/GPU/training；5/5 DOI verified |
| 2026-08-07 | XID-01 finite autoregressive interaction-identifiability theorem | 匹配 \(N\)、factor marginals 与 unseen target 时，diagnostic support 是否相对 redundant support 识别共享 interaction rule | plan `3e8ea53`; result this commit | binary visual factor × three-level language factor；two-rule hypothesis class；finite NLL/minimax proposition；deterministic exhaustive verifier | 两设计 \(N=4\)、V marginals=`2/2`、L marginals=`2/1/1`、target absent；redundant minimax error=`0.5`；identifying unique NLL gap 与解析式在 3 个 eta 全一致 | `VALID` | toy proposition only；`experiments/results/XID-01_round1/`；0 checkpoint/GPU/training/final confirmation |
| 2026-08-07 | XID-01 finite-hypothesis target-risk decomposition | source ERM target excess risk 能否分为 approximation、alignment、exact identification diameter 与 estimation expansion | plan `6ff4298`; result this commit | finite bounded-loss class；uniform convergence；fixed 5-point risk grid × 3 deviation radii；round1 specialization | theorem proven；1,147,625 target tables / 1,530,375 ERM cases、0 violations；round1 redundant 只保留 \(I>0\)，identifying 四项为 0 | `VALID` | `experiments/results/XID-01_round2/`；通用 theory bridge，不是可计算 LVLM certificate；0 checkpoint/GPU/training |
| 2026-08-07 | XID-01 diagnostic-mass prediction theorem | diagnostic mass/separation 能否以预声明阈值排除 target-bad shortcuts，并区分 redundancy 与 identification | plan `b525e7e`; result this commit | \(\lambda,\gamma,\beta,\alpha_n\) sufficient threshold；sharpness construction；fixed rational grid；round1 mapping | theorem + sharpness proven；180 parameter cases、2,700 admissible gaps、68 positive / 112 sharpness cases、0 violations；round1 exact recovery | `VALID` | `experiments/results/XID-01_round3/`；真实 LVLM beta/gamma 未验证；0 checkpoint/GPU/training |
| 2026-08-07 | XID-01 matched-support MiniMind-V mechanism pilot | matched pixels/marginals/entropy/compute 下，consistent interaction support 是否优于 ambiguous visual-necessary support 泛化到 held-out marker×key | plan `5cbe500`; implementation/preflight `cf815e4`; smoke `64ec628`; result this commit | common 10k captions + 1,040 ten-row blocks；M2-current；ambiguous vs consistent；root 43301；1,008-group target + factorial panel | all engineering gates pass；primary consistent−ambiguous=`-48.21 pp`、CI `[-51.69,-44.84] pp`；mechanism diff=`+0.66 pp`，两条件 full-rule success=0 | `INSTANTIATION_REJECTED` | `experiments/results/XID-01_round4/`；只否定当前 empirical instantiation；禁止 roots 43302/43303 和任何 task/ratio/LR/marker/panel rescue |
| 2026-08-07 | LITMAP-07 XID failure-targeted mechanism search | key-specific visual absorption、shared factorization 与 representation/trainability ceiling 中哪一个能产生新 theory/prediction | plan `46fb846`; result this commit | 16 frozen query families；2,455 raw / 1,242 unique；11 篇 primary full text；theorem/setting/limitation audit | 选择 `VSTATE-01` shared visual-state mediation；direct VLM evidence + adjacent gradient theorem 支持建立新 bridge，ceiling 保留为竞争解释 | `VALID` | `experiments/results/LITMAP-07_round1/`；0 GPU/checkpoint/training/final confirmation；无配图 |
