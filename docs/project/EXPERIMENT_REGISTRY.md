...
| 2026-08-07 | XID-01 finite autoregressive interaction-identifiability theorem | 匹配 \(N\)、factor marginals 与 unseen target 时，diagnostic support 是否相对 redundant support 识别共享 interaction rule | current plan commit | binary visual factor × three-level language factor；two-rule hypothesis class；finite NLL/minimax proposition；deterministic exhaustive verifier | immutable plan 由本次 commit 冻结；执行前不写 theorem/proof，不运行 verifier | `PROPOSED` | `experiments/plans/XID-01_round1.md`；0 checkpoint/GPU/training/final confirmation |
