#!/usr/bin/env python3
"""Build the deterministic index for the five frozen LITMAP-05 query families."""
...
QUERY_FAMILIES = {
    "formal_identifiability",
    "formal_identifiability_sufficiency_expansion",
    "formal_identifiability_vinfo_expansion",
    "frozen_sufficiency",
    "frozen_sufficiency_expansion",
    "lvpm_interface",
    "lvpm_interface_probe_expansion",
    "rotation_invariance",
    "rotation_invariance_probe_expansion",
    "vlm_probe",
    "vlm_probe_expansion",
}
...
def family_from_path(path: Path) -> str:
    match = re.match(r"research_litmap05_(.+)_(?:arxiv|openalex)$", path.stem)
    if not match:
        raise ValueError(f"unexpected LITMAP-05 filename: {path}")
    return match.group(1)


def discovery_paths(source_dir: Path, backend: str) -> list[Path]:
    paths = sorted(source_dir.glob(f"research_litmap05_*_{backend}.*"))
...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("experiments/results/LITMAP-05_round1/SEARCH_INDEX.tsv"),
    )
    ...
    if not rows:
        raise RuntimeError("no LITMAP-05 discovery records found")
