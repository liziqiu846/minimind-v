#!/usr/bin/env python3
"""Plan-bound wrapper around the unchanged previous development evaluator."""
...
DEVELOPMENT_PROTOCOL_ID = (
    "phase3-v6-state-dependent-marginal-development-reuse-v1"
)


def _adapted_manifest(path: Path):
    """Supply the legacy evaluator's unused common-anchor argument."""
...
def evaluate(
    *,
    ...
    """Use the previous scorer and risk arithmetic with only plan adaptation."""
    reused.verify_formal_run_plan = _adapted_manifest
    reused._training_config = lambda run, _anchor_dimensions: training_config(run)
