#!/usr/bin/env python3
"""Dry-run or execute the frozen 6-reuse/12-training final experiment."""
...
from . import ACTION_MODULES, EVALUATION_ROLE
from .manifest import DEFAULT_MANIFEST, verify_frozen_manifest
from .runtime import execute_run

DEFAULT_RESULTS_ROOT = Path(
    "/home/lizhaohui/lzq/stage3-language-vision-switch-final-20260805"
)
...
def select_runs(
    ...
    *,
    ...
    if run_id is not None:
        ...
        selected = [
            run for run in runs if run["base_state"] == state
        ]
    else:
        selected = runs
    if seed is not None:
    ...
    if run_type is not None:
        selected = [
            run for run in selected if run["run_type"] == run_type
        ]
    if not selected:
...
def validate_result(
    ...
    if (
        set(bits) != {"vision", "projector", "language"}
        or any(
        ...
        or any(
            result[f"{module}_encoded_bits"] != bits[module]
            for module in bits
        )
    ):
    ...
    if run["module"] is None:
        if result["target_module_encoded_bits"] is not None:
            raise ValueError("base result selects target-module bits")
        if result.get("training_status") != "reused_complete_base_result":
            raise ValueError("base result was not completely reused")
    else:
        if result["target_module_encoded_bits"] != bits[run["module"]]:
            raise ValueError("candidate target bits differ from module codec")
        if result.get("training_status") != "new_training":
            raise ValueError("candidate was not newly trained")
...
def dispatch(
    ...
    *,
    ...
    for requested in selected_runs:
        ...
        if (
            run_id not in authoritative
            or dict(requested) != authoritative[run_id]
        ):
    ...
    receipt = {
        ...
        "selected_reused_complete_base_result_count": sum(
            run["complete_result_reuse"] for run in selected
        ),
...
def main() -> int:
    ...
    parser.add_argument(
        "--run-type",
        choices=("reused_base_result", "new_candidate"),
    )
