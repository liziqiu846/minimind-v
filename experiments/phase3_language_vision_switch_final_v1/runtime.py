"""Reuse/train, encode, freeze-check, and development-evaluate one run."""
...
from . import COORDINATE_MODULES, EVALUATION_ROLE
...
def formal_binding(
    ...
    return {
        ...
        "source_state_experiment_manifest_sha256": manifest[
            "source_experiment"
        ]["manifest_sha256"],
    ...
    *,
    ...
    if result_path.exists():
        ...
        command = [
            ...
            "-m",
            "experiments.phase3_language_vision_switch_final_v1.development_eval",
            "--plan",
    ...
    if (
        ...
        or payload.get("curve_run_plan_sha256")
    ...
    return {
        ...
        "development_protocol_sha256": payload[
        ...
        "development_record_count": int(
            payload["development_record_count"]
        ),
...
def _reuse_complete_base_result(
    run: Mapping[str, Any], binding: Mapping[str, Any]
) -> dict[str, Any]:
    source = run["source"]
    source_path = Path(source["source_run_result_path"])
    if sha256_file(source_path) != source["source_run_result_sha256"]:
        raise ValueError("source base result changed")
    ...
    for module in COORDINATE_MODULES:
        ...
        if sha256_file(path) != source["module_codec_sha256"][module]:
            raise ValueError("source base codec changed")
    if (
        payload["checkpoint_sha256"] != source["checkpoint_sha256"]
        or payload["training_manifest_sha256"]
        != source["training_manifest_sha256"]
        or payload["development_evaluation_result_sha256"]
        != source["development_evaluation_result_sha256"]
        or payload["coordinate_dimensions"]
        != run["coordinate_dimensions"]
    ):
        raise ValueError("source base result binding differs")
    return {
        **binding,
        "schema_version": 1,
        ...
        "vision_encoded_bits": payload["vision_encoded_bits"],
        "projector_encoded_bits": payload["projector_encoded_bits"],
        "language_encoded_bits": payload["language_encoded_bits"],
        "module_wise_encoded_bits": dict(
            payload["module_wise_encoded_bits"]
        ),
        "target_module_encoded_bits": None,
        "total_encoded_bits": int(payload["total_encoded_bits"]),
        "module_codec_paths": dict(payload["module_codec_paths"]),
        "codec_receipt_path": payload["codec_receipt_path"],
        "evaluation_role": payload["evaluation_role"],
        "development_task_risk": float(
            payload["development_task_risk"]
        ),
        "semantic_risk_bound": float(payload["semantic_risk_bound"]),
        "visual_gain_guardrail": float(payload["visual_gain_guardrail"]),
        "development_protocol_id": payload["development_protocol_id"],
        ...
        "development_input_sha256": payload[
        ...
        "development_image_count": int(payload["development_image_count"]),
        "development_evaluation_result_path": payload[
            "development_evaluation_result_path"
        ],
        "development_evaluation_result_sha256": payload[
            "development_evaluation_result_sha256"
        ],
        "source_run_id": source["source_run_id"],
        "source_run_result_path": str(source_path),
        "source_run_result_sha256": source[
            "source_run_result_sha256"
        ],
        "training_status": "reused_complete_base_result",
        "inference_status": "reused_complete_base_result",
        "run_status": "complete",
        "status": "complete",
    }
...
def execute_run(
    ...
    *,
    ...
    if run["run_type"] == "reused_base_result":
        return {
            **_reuse_complete_base_result(run, binding),
    ...
    return {
        **binding,
        ...
        "target_module_encoded_bits": module_bits[target_module],
        ...
        **risk,
        ...
        "inference_status": "new_development_evaluation",
        "run_status": "complete",
        "status": "complete",
    }
